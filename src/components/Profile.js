...
const Profile = () => {
        ...
        axios.get(`${URL}/protected`,{
          headers : {
            ...
            axios.post(`${URL}/analatics`,{id:res.data.user.id}).then(resp => {
                ...
                setUserdata([resp.data.userdata.notes,resp.data.userdata.todos,resp.data.userdata.completedTodos]);
            }).catch(err => {
                console.log(err);
            })
 ...
 const data = {
        labels: [ 'Notes', 'To-do\'s', 'Completed to-do\'s'],
        datasets: [
          {
            label: '# of Votes',
            data:userdata,
    ...
    return (
        <>
            ...
            <div style={{background: "linear-gradient(90deg, rgba(75, 106, 160, 1), rgba(223, 228, 242, 1))"}} className="container d-lg-flex justify-content-evenly align-items-center border shadow p-5 rounded">
                ...
                <p>A complete analysis about number of Notes you have added. Also the count of to-do's and completed to-do's
